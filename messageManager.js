...
class MessageManager {
    ...
    initNewSession() {
        ...
        this.currentSessionLog = path.join(this.logsDir, `session_${sessionId}.log`);
    ...
    async loadContacts() {
        ...
        return fs.readFileSync(numerosPath, "utf8")
            .split("\n")
            .map(n => n.trim())
    ...
    async loadMessage() {
        ...
        if (!fs.existsSync(mensagemPath)) {
            ...
            return "Olá, esta é uma mensagem de teste!";
        }
    ...
    logMessageSent(number, success = true, error = null) {
        ...
        if (success) {
            this.stats.success++;
            this.writeToLog(`✅ ${formatarDataHora(new Date())} | Mensagem enviada para: ${number}`);
        } else {
            this.stats.failed++;
            ...
            this.writeToLog(`❌ ${formatarDataHora(new Date())} | Falha ao enviar para: ${cleanNumber}${errorMsg}`);
        }
    }
    ...
    finalizeSession() {
        ...
        const successPercent = this.stats.total > 0 
            ? Math.round((this.stats.success / this.stats.total) * 100) 
            : 0;
        const failedPercent = this.stats.total > 0 
        ...
        this.writeToLog(`✅ Total de mensagens enviadas com sucesso: ${finalStats.success} (${finalStats.successPercent}%)`);
        this.writeToLog(`❌ Total de mensagens não enviadas: ${finalStats.failed} (${finalStats.failedPercent}%)`);
